...
export default async function ShopPage({ searchParams }: ShopPageProps) {
  ...
  if (
    ...
    initialQuery.sale_unit = saleUnit;
  ...
  if (plantSize === "small" || plantSize === "medium" || plantSize === "large")
    initialQuery.plant_size = plantSize;
  const grade = value("quality_grade");
  if (grade === "standard" || grade === "premium" || grade === "luxury")
    initialQuery.quality_grade = grade;
  const flowerGrade = value("flower_grade");
  if (
    flowerGrade === "standard" ||
    flowerGrade === "premium" ||
    flowerGrade === "luxury"
  )
    initialQuery.flower_grade = flowerGrade;
  const care = value("care_difficulty");
  if (care === "easy" || care === "medium" || care === "hard")
    initialQuery.care_difficulty = care;
  ...
  if (
    ...
    initialQuery.light_requirement = light;
  const watering = value("watering_requirement");
  if (watering === "low" || watering === "medium" || watering === "high")
    initialQuery.watering_requirement = watering;
  for (const key of ["pet_friendly", "pot_included", "has_drainage"] as const)
    initialQuery[key] = booleanValue(key);
  for (const key of [
    "pot_material",
    "pot_color",
    "flower_type",
  ...
  if (
    ...
    initialQuery.fragrance_level = fragrance;
  ...
  if (
    ...
    initialQuery.seasonal_availability = season;

  return <ShopExperience initialQuery={initialQuery} />;
}
