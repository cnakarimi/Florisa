...
export function OrderDetailExperience({
  ...
  useEffect(() => {
    ...
    if (!auth.isAuthenticated) {
      router.replace(
        `/auth?next=${encodeURIComponent(`/orders/${publicNumber}${success ? "/success" : ""}`)}`,
      );
      return;
    }
    if (!auth.isProfileComplete) {
      router.replace(
        `/auth/register?next=${encodeURIComponent(`/orders/${publicNumber}${success ? "/success" : ""}`)}`,
      );
      return;
    }
    getOrder(publicNumber)
      .then(setOrder)
        ...
        {order ? (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => router.push(`/orders/${order.public_number}`)}
...
function Row({
  ...
  return (
    <div
      className={`flex justify-between text-sm ${strong ? "font-black" : "text-zinc-400"}`}
    >
      <span>{label}</span>
